...
export class TrackerAssignmentCreateDto {
    @ApiProperty({
    ...
    @ApiProperty({
        description: 'شماره سیم‌کارت دستگاه',
    ...
    @ApiProperty({
        description: 'شناسه وسیله نقلیه',
        example: 1
    })
    @IsNotEmpty({ message: 'شناسه وسیله نقلیه نمی‌تواند خالی باشد' })
    @IsNumber({}, { message: 'شناسه وسیله نقلیه باید عدد باشد' })
    @Type(() => Number)
    vehicleId: number;
...
export interface TrackerAssignmentCreateDomainDto {
